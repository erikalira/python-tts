...
class MockConfigRepository(IConfigRepository):
    ...
    def __init__(self):
        self.configs = {}
        self.user_configs = {}
        self.default_config = TTSConfig(engine="gtts", language="pt", voice_id="roa/pt-br", rate=180)
    ...
    def set_config(self, guild_id: int, config: TTSConfig, user_id: int | None = None) -> None:
        ...
        target = TTSConfig(engine=config.engine, language=config.language, voice_id=config.voice_id, rate=config.rate)
...
class MockTTSCatalog:
    ...
    def is_voice_available(self, *, engine: str, voice_id: str) -> bool:
        return any(
            option.engine == engine and option.voice_id.lower() == voice_id.lower() for option in self.options.values()
        )
...
def sample_tts_request():
    ...
    return TTSRequest(text="Hello world", channel_id=123456, guild_id=789012, member_id=345678)


@pytest.fixture
def sample_tts_config():
    """Fixture for sample TTS config."""
    return TTSConfig(engine="gtts", language="en", voice_id="en-us", rate=180)
