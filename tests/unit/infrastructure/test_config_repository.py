...
class TestInMemoryConfigRepository:
    ...
    def test_get_config_for_unknown_user(self):
        """Test getting config for user without custom settings."""
        default_config = TTSConfig(engine='gtts', language='pt')
        repo = InMemoryConfigRepository(default_config)
        
        config = repo.get_config(user_id=999)
    ...
    def test_set_and_get_user_config(self):
        """Test setting and getting user-specific config."""
        default_config = TTSConfig(engine='gtts', language='pt')
        repo = InMemoryConfigRepository(default_config)
        
        # Set custom config for user
        custom_config = TTSConfig(engine='pyttsx3', language='en')
        repo.set_config(user_id=123, config=custom_config)
        
        # Retrieve it
        config = repo.get_config(user_id=123)
        
        assert config.engine == 'pyttsx3'
        assert config.language == 'en'
    
    def test_multiple_user_configs(self):
        """Test managing configs for multiple users."""
        default_config = TTSConfig(engine='gtts', language='pt')
        repo = InMemoryConfigRepository(default_config)
        
        # Set config for user 1
        config1 = TTSConfig(engine='gtts', language='en')
        repo.set_config(user_id=1, config=config1)
        
        # Set config for user 2
        config2 = TTSConfig(engine='pyttsx3', language='es')
        repo.set_config(user_id=2, config=config2)
        
        # Verify each user has correct config
        assert repo.get_config(user_id=1).language == 'en'
        assert repo.get_config(user_id=2).language == 'es'
        assert repo.get_config(user_id=3).language == 'pt'  # Default
