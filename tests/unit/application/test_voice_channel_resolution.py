...
class TestVoiceChannelResolutionService:
    async def test_infer_guild_id_from_member_channel(self):
        ...
        service = VoiceChannelResolutionService(ResolutionRepository(member_channel=member_channel))
    ...
    async def test_reuses_connected_channel_when_member_is_already_there(self):
        ...
        service = VoiceChannelResolutionService(ResolutionRepository(member_channel=channel, connected_channel=channel))

        result = await service.resolve_for_request(TTSRequest(text="test", guild_id=789012, member_id=345678))
    ...
    async def test_rejects_explicit_channel_without_member_context(self):
        ...
        result = await service.resolve_for_request(TTSRequest(text="test", guild_id=789012, channel_id=123456))
