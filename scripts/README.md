...
[SMALL_CLOUD_VM_DEPLOY.md](../docs/deploy/SMALL_CLOUD_VM_DEPLOY.md).

```bash
# Install runtime configuration and deployment assets on the instance
./scripts/deploy/vm-bootstrap-env.sh --host <PUBLIC_IP> --env-file ./.env.oci
...
- `deploy/vm-bootstrap-env.sh`: uploads the runtime env file with mode 0600,
  plus the compose file, Caddyfile, and deploy script. Refuses placeholder
  secrets. Never disables SSH host key verification.
- `deploy/vm-deploy.sh`: validates the tag, verifies the GHCR manifest has a
